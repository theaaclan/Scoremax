...
return new class extends Migration
{
    ...
    {
        Schema::create('volleyball_brackets', function (Blueprint $table) {
            $table->id('MatchID');  // Primary key for volleyball_matches table
...
$table->foreign('LeagueID')->references('LeagueID')->on('leagues')->cascadeOnDelete();
            ...
            $table->foreign('Team1ID')
                ->references('TeamID')  // Reference TeamID in volleyballteams
                ->on('volleyballteams')
    ...
    {
        Schema::dropIfExists('volleyball_brackets');
    }
};
