...
const BaseballPlayers = ({ players: initialPlayers = [], teams, auth }) => {
    ...
    const handleDropdownChange = (criteria) => {
        ...
        const getPlayerByCriteria = (criteria) => {
            ...
            return matchDetails.reduce((max, player) => {
                ...
                if (!statKey && criteria !== "highestTotal") return null;
        ...
        console.log("Selected Player:", resultPlayer); // Debugging here
        ...
        if (resultPlayer) {
            ...
            const statValue =
                criteria === "highestTotal"
                    ? (resultPlayer.Runs || 0) +
                      (resultPlayer.Hits || 0) +
                      (resultPlayer.Home_Runs || 0) +
                      (resultPlayer.Strikeouts || 0) +
                      (resultPlayer.Errors || 0)
                    : resultPlayer[statKey] || 0;

            setSelectedStat(
                `${resultPlayer.FullName} (${statKey}: ${statValue})`
            );
        } else {
    ...
    const handleSubmit = (e) => {
        ...
        if (selectedPlayer) {
            // Update Player
            axios
                .put(`/baseballplayers/${selectedPlayer.PlayerID}`, form)
                .then((response) => {
                    setPlayers((prev) =>
                        prev.map((player) =>
    ...
    const historyColumns = [
        {
            name: "Player Name",
            selector: (row) => row.FullName, // Changed from PlayerName to FullName
            sortable: true,
        },
        { name: "Team Name", selector: (row) => row.TeamName, sortable: true },
        { name: "Runs", selector: (row) => row.Runs, sortable: true },
        { name: "Hits", selector: (row) => row.Hits, sortable: true },
        { name: "Home_Runs", selector: (row) => row.Home_Runs, sortable: true },
        {
            name: "Strikeouts",
            selector: (row) => row.Strikeouts,
            sortable: true,
        }, // Changed from Blocks to Strikeouts
        { name: "Errors", selector: (row) => row.Errors, sortable: true },
        {
            name: "Date",
    ...
    const exportToExcel = () => {
        ...
        XLSX.writeFile(workbook, "Baseball_Players.xlsx");
    };
    ...
    const columnsMatchDetails = [
        ...
        { title: "Hits", dataKey: "Hits" },
        { title: "Home_Runs", dataKey: "Home_Runs" },
        { title: "Strikeouts", dataKey: "Strikeouts" },
        { title: "Errors", dataKey: "Errors" },
    ];
    ...
    const handlePrintPlayersPDF = () => {
        ...
        doc.setFontSize(18);
        doc.text("Baseball Players List", 14, 10); // Title
                                    ...
                                    <div className="modal-body">
                                        ...
                                        <div className="d-flex align-items-center mb-3">
                                            ...
                                            >
                                                ...
                                                <option value="highestRuns">
                                                    Highest Runs
                                                </option>
                                                <option value="highestHits">
                                                    Highest Hits
                                                </option>
                                                <option value="highestHome_Runs">
                                                    Highest Home Runs
                                                </option>
                                                <option value="highestStrikeouts">
                                                    Highest Strikeouts
                                                </option>
                                                <option value="highestErrors">
                                                    Highest Errors
                                                </option>
